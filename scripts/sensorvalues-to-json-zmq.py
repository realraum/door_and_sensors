...
lh_syslog.setFormatter(logging.Formatter(
    'sensorvalues-to-json-zmq.py: %(levelname)s %(message)s'))
...
class UWSConfig:
    ...
    def __init__(self, configfile=None):
        ...
        self.config_parser.set(
            'json',
            'write_path',
            "/dev/shm/wget/r3sensors.json")
        self.config_parser.set(
            'json',
            'moveto_path',
            "/dev/shm/www/r3sensors.json")
        self.config_parser.set(
            'json',
            'backup_path',
            "/home/guests/realraum.wirdorange.org/r3sensors.json.bak")
        ...
        self.config_parser.set(
            'zmq',
            'remote_uri',
            "tcp://zmqbroker.realraum.at:4244")
        self.config_parser.set(
            'zmq',
            'sshtunnel',
            "realraum@zmqbroker.realraum.at:22000")
        self.config_parser.set(
            'zmq',
            'sshkeyfile',
            "/home/guests/realraum.wirdorange.org/id_rsa")
        self.config_parser.set(
            'zmq',
            'subscribe',
            "TempSensorUpdate IlluminationSensorUpdate DustSensorUpdate RelativeHumiditySensorUpdate MovementSensorUpdate")
...
latest_values_ = {}
sensor_store_ = {}
# stores number of columns for a sensor not counting Time (x-axis) column.
# AKA the number of data-rows. Equals highest SensorIndex +1
sensor_cols_num_ = {}
...
def addEventToTempLastValueStore(structname, msgdata):
    ...
    sensorindex = int(msgdata["Sensorindex"]
                      ) if "Sensorindex" in msgdata else 0
    if structname not in latest_values_:
        latest_values_[structname] = []
    if not structname in sensor_cols_num_ or sensor_cols_num_[
            structname] < sensorindex + 1:
        sensor_cols_num_[structname] = sensorindex + 1
    if len(latest_values_[structname]) < sensor_cols_num_[structname]:
        latest_values_[structname] += [0] * \
            (sensor_cols_num_[structname] - len(latest_values_[structname]))
    ...
    if len(msgdata) > 0:
        ...
        # if that value does not exist, (i.e. movementevent), count event
        # occurances
...
def expandSensorStoreLists(structname, newlength):
    ...
    if structname not in sensor_store_:
    ...
    sensor_store_[structname] = map(lambda l: l[:newlength_including_time] + (
        [0] * (newlength_including_time - len(l))), sensor_store_[structname])
...
def addEventsToStore():
    ...
    for structname in latest_values_.keys():
        ...
        if len(sensor_store_[structname]) == 0 or len(sensor_store_[structname][
                0]) < 2 or sensor_store_[structname][0][0] != time_column_name_:
            sensor_store_[structname].insert(
                0,
                [time_column_name_] +
                list(
                    map(
                        ...
                        range(
                            0,
                            sensor_cols_num_[structname]))))

        # add values
        try:
            # if latest values are identical, just update timestamp
            if sensor_store_[structname][-1][1:] == latest_values_[structname] and sensor_store_[
                    structname][-1][1:] == sensor_store_[structname][-2][1:]:
        ...
        if uwscfg.json_limit_list_len:
            if len(sensor_store_[structname]) > uwscfg.json_limit_list_len:
                sensor_store_[structname] = sensor_store_[
                    structname][- uwscfg.json_limit_list_len:]
        ...
        try:
            ...
            if uwscfg.zmq_sshtunnel:
                zmq.ssh.tunnel.tunnel_connection(
                    zmqsub,
                    uwscfg.zmq_remote_uri,
                    uwscfg.zmq_sshtunnel,
                    keyfile=uwscfg.zmq_sshkeyfile)
            else:
            ...
            while True:
                ...
                if int(time.time()) - \
                        last_update < int(uwscfg.json_updateinterval):
                    continue
                ...
                if backup_counter > uwscfg.json_backup_every:
                    backup_counter = 0
                    shutil.copy(
                        uwscfg.json_write_path,
                        uwscfg.json_backup_path)
