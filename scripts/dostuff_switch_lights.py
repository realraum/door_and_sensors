...
def onMqttMessage(client, userdata, msg):
    ...
    try:
        ...
        if topic.endswith("/duskordawn") and "HaveSunlight" in dictdata:
            ...
            if boredoomcount_ == 0:
                client.publish("action/PipeLEDs/pattern","{\"pattern\":\"rainbow\",\"arg\":4}")
            else:
                client.publish("action/PipeLEDs/pattern","{\"pattern\":\"spots\",\"arg\":%d}" % (boredoomcount_))
            boredoomcount_ = (boredoomcount_ +1) % 5 #more than 5 spots is not supported
