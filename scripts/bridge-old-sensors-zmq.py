...
lh_syslog.setFormatter(logging.Formatter(
    'bridge-old-sensors.py: %(levelname)s %(message)s'))
...
class UWSConfig:
    ...
    def __init__(self, configfile=None):
        ...
        self.config_parser.set(
            'sensors',
            'remote_cmd',
            "ssh -i /flash/tuer/id_rsa -o PasswordAuthentication=no -o StrictHostKeyChecking=no %RHOST% %RSHELL% %RSOCKET%")
        self.config_parser.set(
            'sensors',
            'remote_host',
            "root@slug.realraum.at")
        self.config_parser.set(
            'sensors',
            'remote_socket',
            "/var/run/powersensordaemon/cmd.sock")
...
def trackSensorStatus(uwscfg, zmqsocket):
    ...
    while tracksensor_running:
        ...
        sshp = None
        try:
            cmd = uwscfg.sensors_remote_cmd.replace(
                "%RHOST%",
                uwscfg.sensors_remote_host).replace(
                "%RSHELL%",
                uwscfg.sensors_remote_shell).replace(
                "%RSOCKET%",
                uwscfg.sensors_remote_socket).split(" ")
            ...
            sshp = subprocess.Popen(
                ...
                shell=False)
            logging.debug(
                "trackSensorStatus: pid %d: running=%d" %
                (sshp.pid, sshp.poll() is None))
            if not sshp.poll() is None:
                raise Exception(
                    "trackSensorStatus: subprocess %d not started ?, returncode: %d" %
                    (sshp.pid, sshp.returncode))
            #sshp.stdin.write("listen movement\nlisten button\nlisten sensor\n")
            # if we send listen bevor usocket is running, we will never get
            # output
            time.sleep(5)
            ...
            while tracksensor_running:
                if not sshp.poll() is None:
                    raise Exception(
                        "trackSensorStatus: subprocess %d finished, returncode: %d" %
                        (sshp.pid, sshp.returncode))
                line = sshp.stdout.readline()
                if len(line) < 1:
                    raise Exception(
                        "EOF on Subprocess, daemon seems to have quit, returncode: %d",
                        sshp.returncode)
                logging.debug("trackSensorStatus: Got Line: " + line)
                m = RE_BUTTON.match(line)
                if not m is None:
                    sendR3Message(
                        zmqsocket, "BoreDoomButtonPressEvent", {
                            "Ts": int(
                                time.time())})
                    continue
                ...
                if not m is None:
                    sendR3Message(
                        zmqsocket, "MovementSensorUpdate", {
                            "Sensorindex": 0, "Ts": int(
                                time.time())})
                    continue
                m = RE_TEMP.match(line)
                if not m is None:
                    sendR3Message(
                        zmqsocket, "TempSensorUpdate", {
                            "Sensorindex": int(
                                m.group(1)), "Value": float(
                                m.group(2)), "Ts": int(
                                time.time())})
                    continue
                ...
                if not m is None:
                    sendR3Message(
                        zmqsocket, "IlluminationSensorUpdate", {
                            "Sensorindex": int(
                                m.group(1)), "Value": int(
                                m.group(2)), "Ts": int(
                                time.time())})
            ...
            if not sshp is None and sshp.poll() is None:
                ...
                if sshp.poll() is None:
                    logging.error(
                        "trackSensorStatus: subprocess still alive, sending SIGKILL to pid %d" %
                        (sshp.pid))
                    if sys.hexversion >= 0x020600F0:
                        sshp.kill()
                    else:
