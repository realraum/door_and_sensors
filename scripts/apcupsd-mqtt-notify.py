...
def getAPCStatus():
    ...
    try:
        res = subprocess.check_output(
            "/sbin/apcaccess", timeout=8).decode("utf8")
...
if __name__ == '__main__':
    ...
    batt_percent = float(apc_status["bcharge"]
                         ) if "bcharge" in apc_status else 0
    ts = int(time.time())
    if sys.argv[0].endswith("offbattery"):
        sendR3Message(
            client, TOPIC_BACKDOOR_POWERLOSS, {
                "OnBattery": False, "PercentBattery": batt_percent, "Ts": ts}, qos=2)
    elif sys.argv[0].endswith("onbattery"):
        sendR3Message(
            client, TOPIC_BACKDOOR_POWERLOSS, {
                "OnBattery": True, "PercentBattery": batt_percent, "Ts": ts}, qos=2)
    else:
        sendR3Message(client,
                      TOPIC_BACKDOOR_POWERLOSS,
                      {"OnBattery": apc_status["status"] != "ONLINE",
                       "PercentBattery": batt_percent,
                       "LineVoltage": float(apc_status["linev"]),
                       "LoadPercent": float(apc_status["loadpct"]),
                       "Ts": int(time.time())},
                      qos=0)
        sendR3Message(client,
                      TOPIC_BACKDOOR_BATTTEMP,
                      {"Location": "UPS " + apc_status["upsname"] + " Battery",
                       "Value": float(apc_status["itemp"]),
                          "Ts": ts},
                      qos=0)
