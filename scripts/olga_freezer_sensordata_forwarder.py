...
class UWSConfig:
    ...
    def __init__(self, configfile=None):
        ...
        self.config_parser.set(
            'sensor', 'uri', "http://olgafreezer.realraum.at/")
...
def sendSMS(groups, message):
    ...
    smsproc = subprocess.Popen(
        ["/usr/local/bin/send_group_sms.sh"] + groups,
    ...
    if "sensors" in jsondict:
        ...
        for tsd in jsondict["sensors"]:
            ...
            if ts - last_publish_ts[loc] > int(uwscfg.sensor_publishinterval):
                ...
                sendR3Message(mqttclient,
                              "realraum/olgafreezer/temperature",
                              {"Location": loc,
                               "Value": temp,
                               "Ts": ts},
                              retain=True)
        ...
        if unreachable_count >= int(uwscfg.sensor_warnunreachablelimit) and unreachable_count < 2 * int(uwscfg.sensor_warnunreachablelimit):
            ...
            if (unreachable_count - int(uwscfg.sensor_warnunreachablelimit)) % 2 == 0:
                print(
                    "send RF Code for olgatherm OFF (%s)" %
                    time.strftime("%c"))
                sendR3Message(mqttclient,
                              "action/rf433/sendcode3byte",
                              {"Code": list(b"\x00\xa2\x2a"),
                               "Ts": ts},
                              qos=1,
                              retain=False)
            else:
                print(
                    "send RF Code for olgatherm ON (%s)" %
                    time.strftime("%c"))
                sendR3Message(mqttclient,
                              "action/rf433/sendcode3byte",
                              {"Code": list(b"\x00\xa2\x8a"),
                               "Ts": int(time.time())},
                              qos=1,
                              retain=False)
            ...
            sendR3Message(mqttclient,
                          ...
                          {"Topic": "realraum/olgafreezer/temperature",
                           "LastSeen": ts - int(uwscfg.sensor_sampleinterval) * int(uwscfg.sensor_warnunreachablelimit),
                              "Ts": ts})
        unreachable_count += 1
